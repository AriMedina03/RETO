...
osThreadId_t TaskHandleESC;
const osThreadAttr_t TaskESC_attributes = {
  .name = "TaskESC",
  .stack_size = 128 * 4,
  .priority = (osPriority_t) osPriorityNormal,
};
...
void taskServo(void *argument)
{
  ...
  for(;;)
  {
	  htim3.Instance-> CCR2 = (1.5*65535)/20;
	  osDelay(2000);

	  htim3.Instance -> CCR2 = (2*65535)/20;
	  osDelay(10000);
...
void taskESC(void *argument)
{
  ...
  for(;;)
  {
	  ...
	  while(pulseWidth<1.6){
		  htim2.Instance -> CCR1= (pulseWidth*65535)/20;
		  osDelay(50);
		  pulseWidth += i;
	  }
...
/*
  ...
  void vAFunction (StreamBufferHandle_t xStreamBuffer){
	  // Declare variables
	  size_t xBytesSent;
	  uint8_t ucNumToSed[4];
	  // Copy data from 'acum' to 'ucNumToSed'
	  memcpy(ucNumToSed, &acum, sizeof(acum));
      // Define a constant for the timeout period (100 milliseconds)
	  const TickType_t x100ms = pdMS_TO_TICKS(100);
      // Send data to the stream buffer
	  xBytesSent = xStreamBufferSend (xStreamBuffer, (void *)ucNumToSed, sizeof(ucNumToSed), x100ms);
  }
