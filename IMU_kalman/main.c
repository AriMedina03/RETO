...
MX_USART3_UART_Init();
  ...
  /* USER CODE BEGIN 2 */
	 ...
	 int16_t accelX_data, accelY_data, accelZ_data;
  ...
  while (1)
  {
	  ...
	  //accelX_data = ((int16_t)imu_data[0]<<8) + imu_data[1];
	  //accelY_data = ((int16_t)imu_data[2]<<8) + imu_data[3];
	  //accelZ_data = ((int16_t)imu_data[4]<<8) + imu_data[5];

	  //predicciones de ángulos para saber la dirección
	  AngX=MPU9250.KalmanAngleX;
	  AngY=MPU9250.KalmanAngleY;
